...
void TranslateNativeSegment(NativeGrindSegment &segment, Vec3 translation) {
...
SkateObjectAsset ExtractSkateObjectAsset(MapDefinition package) {
  const bool version_2 =
      package.package_version == 14 && package.map_object_schema_version == 3;
  if (package.package_version == 14 && !version_2) {
    throw std::runtime_error("SKATEOBJ v2 requires SKATE14 with MOBJ schema 3");
  }
  if (package.editable_objects.empty() ||
      (!version_2 && package.editable_objects.size() != 1)) {
    throw std::runtime_error(
        version_2 ? "SKATEOBJ v2 must contain at least one prefab root object"
  ...
  std::vector<bool> claimed_indices(package.render_mesh.indices.size(), false);
  std::vector<bool> claimed_collision(package.collision_triangles.size(),
                                      false);
  ...
  for (const MapObject &source : package.editable_objects) {
    ...
    const std::size_t collision_begin = source.source_first_collision_triangle;
    const std::size_t collision_count = source.source_collision_triangle_count;
    if (index_begin > claimed_indices.size() ||
      ...
      throw std::runtime_error("SKATEOBJ root ownership range is invalid");
    }
    std::fill(claimed_indices.begin() +
    ...
    std::fill(
        ...
        true);
    for (const std::uint32_t rail_index : source.grind_rail_indices) {
      if (rail_index >= claimed_rails.size() || claimed_rails[rail_index]) {
        throw std::runtime_error("SKATEOBJ grind ownership is invalid");
      }
  ...
  if (std::find(claimed_indices.begin(), claimed_indices.end(), false) !=
          claimed_indices.end() ||
      std::find(claimed_collision.begin(), claimed_collision.end(), false) !=
  ...
  for (MapObject &object : asset.objects) {
    ...
    object.source_collision_triangle_count =
        static_cast<std::uint32_t>(object.collision_triangles.size());
  }
  for (GrindRail &rail : asset.grind_rails) {
    ...
    for (NativeGrindSegment &segment : rail.native_segments) {
      TranslateNativeSegment(
          segment, {-authored_pivot.x, -authored_pivot.y, -authored_pivot.z});
    }
  }
  return asset;
}
...
void RemapSkateObjectBreakGroups(SkateObjectAsset &asset,
  ...
  for (MapObject &object : asset.objects) {
    ...
    if (next_group == std::numeric_limits<std::uint32_t>::max()) {
      throw std::runtime_error("SKATEOBJ break-group namespace is exhausted");
    }
...
SkateObjectAsset LoadSkateObjectPackage(std::span<const std::uint8_t> bytes) {
  return ExtractSkateObjectAsset(LoadOwnedMapPackage(bytes));
}
