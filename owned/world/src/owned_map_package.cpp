...
constexpr std::array<char, 8> kMagicV1 = {'S', 'K', 'A', 'T',
                                          'E', '0', '1', '\0'};
constexpr std::array<char, 8> kMagicV2 = {'S', 'K', 'A', 'T',
                                          'E', '0', '2', '\0'};
constexpr std::array<char, 8> kMagicV3 = {'S', 'K', 'A', 'T',
                                          'E', '0', '3', '\0'};
constexpr std::array<char, 8> kMagicV4 = {'S', 'K', 'A', 'T',
                                          'E', '0', '4', '\0'};
constexpr std::array<char, 8> kMagicV5 = {'S', 'K', 'A', 'T',
                                          'E', '0', '5', '\0'};
constexpr std::array<char, 8> kMagicV6 = {'S', 'K', 'A', 'T',
                                          'E', '0', '6', '\0'};
constexpr std::array<char, 8> kMagicV7 = {'S', 'K', 'A', 'T',
                                          'E', '0', '7', '\0'};
constexpr std::array<char, 8> kMagicV8 = {'S', 'K', 'A', 'T',
                                          'E', '0', '8', '\0'};
constexpr std::array<char, 8> kMagicV9 = {'S', 'K', 'A', 'T',
                                          'E', '0', '9', '\0'};
constexpr std::array<char, 8> kMagicV10 = {'S', 'K', 'A', 'T',
                                           'E', '1', '0', '\0'};
constexpr std::array<char, 8> kMagicV11 = {'S', 'K', 'A', 'T',
                                           'E', '1', '1', '\0'};
constexpr std::array<char, 8> kMagicV12 = {'S', 'K', 'A', 'T',
                                           'E', '1', '2', '\0'};
constexpr std::array<char, 8> kMagicV13 = {'S', 'K', 'A', 'T',
                                           'E', '1', '3', '\0'};
constexpr std::array<char, 8> kMagicV14 = {'S', 'K', 'A', 'T',
                                           'E', '1', '4', '\0'};
constexpr std::array<char, 8> kMagicV15 = {'S', 'K', 'A', 'T',
                                           'E', '1', '5', '\0'};
...
constexpr std::uint32_t kMaximumEmbeddedCollisionBytes = 512u * 1024u * 1024u;
constexpr std::uint64_t kMaximumPackageBytes =
    4ull * 1024ull * 1024ull * 1024ull;
constexpr float kAreaEpsilon = 1.0e-10f;
constexpr std::uint32_t kTexturedSkyVertexBytes = sizeof(float) * 5u;

std::uint32_t InferPresentationDepthLayer(const SurfaceMaterial& material) {
  ...
  constexpr std::array<const char*, 13> kOverlayNames = {
      "sign",  "poster",   "billboard", "adbord", "advert", "banner", "logo",
      "decal", "graffiti", "sticker",   "plaque", "letter", "neon"};
...
class Reader {
 public:
  ...
  Vec2 Vector2() { return {Scalar<float>(), Scalar<float>()}; }

  Vec3 Vector3() { return {Scalar<float>(), Scalar<float>(), Scalar<float>()}; }
  ...
  std::span<const std::uint8_t> View(std::size_t size) {
    ...
    const std::span<const std::uint8_t> result(bytes_.data() + offset_, size);
    offset_ += size;
    return result;
  }
...
StoredPayload ReadStoredPayload(Reader& reader, std::size_t expected_size,
                                const char* label) {
  const std::uint32_t method = reader.Scalar<std::uint32_t>();
  const std::uint32_t stored_size = reader.Scalar<std::uint32_t>();
  StoredPayload payload{method, expected_size, reader.ByteVector(stored_size)};
  if (method == kStorageRaw) {
    if (payload.bytes.size() != expected_size) {
      throw std::runtime_error(std::string("SKATE ") + label +
                               " raw size is invalid");
    }
    return payload;
  }
  if (method != kStorageDeflate && method != kStorageZstd &&
      method != kStorageZstdRgbaFilter && method != kStorageZstdVertexSoa &&
...
std::vector<std::uint8_t> InflateBytes(std::span<const std::uint8_t> bytes,
  ...
  const int result = uncompress(decoded.data(), &decoded_size, bytes.data(),
...
std::vector<std::uint8_t> DecompressZstd(std::span<const std::uint8_t> bytes,
                                         ...
                                         const char* label) {
  std::vector<std::uint8_t> decoded(expected_size);
  const std::size_t result = ZSTD_decompress(decoded.data(), decoded.size(),
                                             bytes.data(), bytes.size());
  if (ZSTD_isError(result) || result != expected_size) {
    throw std::runtime_error(std::string("SKATE ") + label +
                             " Zstandard payload is invalid");
  }
  return decoded;
}

std::uint8_t PaethPredictor(std::uint8_t left, std::uint8_t above,
                            std::uint8_t upper_left) {
  const int prediction = static_cast<int>(left) + static_cast<int>(above) -
...
std::vector<std::uint8_t> UndoRgbaFilters(std::vector<std::uint8_t> transformed,
                                          std::size_t expected_size,
                                          const char* label) {
  ...
  if (byte_count != expected_size || width > kMaximumTextureDimension ||
      height > kMaximumTextureDimension) {
    throw std::runtime_error(std::string("SKATE ") + label +
                             " filtered dimensions are invalid");
  }
...
std::vector<std::uint8_t> UndoVertexSoa(std::vector<std::uint8_t> transformed,
                                        std::size_t expected_size,
                                        const char* label) {
  constexpr std::size_t kVertexSize = 56u;
  if (expected_size % kVertexSize != 0 || transformed.size() != expected_size) {
    throw std::runtime_error(std::string("SKATE ") + label +
                             " vertex streams are invalid");
  }
  const std::size_t count = expected_size / kVertexSize;
  constexpr std::array<std::size_t, 7> kFieldSizes{12u, 12u, 8u, 8u,
                                                   4u,  8u,  4u};
  constexpr std::array<std::size_t, 7> kFieldOffsets{0u,  12u, 24u, 32u,
                                                     40u, 44u, 52u};
  std::vector<std::uint8_t> decoded(expected_size);
  std::size_t source_offset = 0;
  for (std::size_t field = 0; field < kFieldSizes.size(); ++field) {
    for (std::size_t index = 0; index < count; ++index) {
      std::memcpy(decoded.data() + index * kVertexSize + kFieldOffsets[field],
                  transformed.data() + source_offset, kFieldSizes[field]);
...
std::uint64_t ReadVaruint(std::span<const std::uint8_t> bytes,
...
std::vector<std::uint8_t> UndoIndexDelta(std::vector<std::uint8_t> transformed,
                                         std::size_t expected_size,
                                         const char* label) {
  ...
  for (std::size_t index = 0; index < count; ++index) {
    const std::uint64_t zigzag = ReadVaruint(transformed, offset, label);
    const std::int64_t delta = static_cast<std::int64_t>(zigzag >> 1) ^
    ...
    if (current < 0 || current > std::numeric_limits<std::uint32_t>::max()) {
      throw std::runtime_error(std::string("SKATE ") + label +
                               " index delta is invalid");
    }
    const auto value = static_cast<std::uint32_t>(current);
    std::memcpy(decoded.data() + index * sizeof(value), &value, sizeof(value));
    previous = current;
  }
  if (offset != transformed.size()) {
    throw std::runtime_error(std::string("SKATE ") + label +
                             " has trailing index data");
  }
  return decoded;
}

std::vector<std::uint8_t> UndoIndexedCollision(
    std::vector<std::uint8_t> transformed, std::size_t expected_size,
    const char* label) {
  constexpr std::size_t kTriangleSize = 48u;
  if (expected_size % kTriangleSize != 0) {
    throw std::runtime_error(std::string("SKATE ") + label +
                             " collision size is invalid");
  }
  Reader reader(std::move(transformed));
  ...
  for (std::size_t triangle = 0; triangle < triangle_count; ++triangle) {
    std::uint8_t* packed = decoded.data() + triangle * kTriangleSize;
    for (std::size_t corner = 0; corner < 3; ++corner) {
      ...
      std::memcpy(packed + corner * sizeof(float) * 3u,
...
std::vector<std::uint8_t> DecodeStoredPayload(StoredPayload payload,
                                              const char* label) {
  ...
  if (!transformed) {
    return payload.method == kStorageDeflate
               ...
               : DecompressZstd(payload.bytes, payload.expected_size, label);
  }
  if (payload.bytes.size() < sizeof(std::uint32_t)) {
    throw std::runtime_error(std::string("SKATE ") + label +
                             " transformed payload is truncated");
  }
  std::uint32_t transformed_size = 0;
  std::memcpy(&transformed_size, payload.bytes.data(),
              sizeof(transformed_size));
  ...
  const bool uses_zstd = payload.method >= kStorageZstdRgbaFilter &&
                         payload.method <= kStorageZstdCollisionIndexed;
  std::vector<std::uint8_t> transformed_bytes =
      uses_zstd ? DecompressZstd(compressed, transformed_size, label)
                : InflateBytes(compressed, transformed_size, label);
  switch (payload.method) {
    case kStorageZstdRgbaFilter:
    case kStorageDeflateRgbaFilter:
      return UndoRgbaFilters(std::move(transformed_bytes),
                             payload.expected_size, label);
    case kStorageZstdVertexSoa:
    case kStorageDeflateVertexSoa:
      return UndoVertexSoa(std::move(transformed_bytes), payload.expected_size,
                           label);
    case kStorageZstdIndexDelta:
    case kStorageDeflateIndexDelta:
      return UndoIndexDelta(std::move(transformed_bytes), payload.expected_size,
                            label);
    case kStorageZstdCollisionIndexed:
    case kStorageDeflateCollisionIndexed:
      return UndoIndexedCollision(std::move(transformed_bytes),
                                  payload.expected_size, label);
    default:
      break;
  }
...
std::vector<std::uint8_t> ReadStoredBytes(Reader& reader,
                                          std::size_t expected_size,
                                          const char* label) {
  return DecodeStoredPayload(ReadStoredPayload(reader, expected_size, label),
                             label);
}

void SkipStoredBytes(Reader& reader, std::size_t expected_size,
                     const char* label) {
  const std::uint32_t method = reader.Scalar<std::uint32_t>();
  const std::uint32_t stored_size = reader.Scalar<std::uint32_t>();
  ...
  if (method != kStorageRaw &&
      (method < kStorageDeflate || method > kStorageDeflateCollisionIndexed)) {
    throw std::runtime_error(std::string("SKATE ") + label +
                             " storage method is unsupported");
  }
...
void ReadRenderVertices(Reader& reader, std::uint32_t count,
  ...
  if (has_retail_channels) {
    const std::span<const std::uint8_t> bytes =
        reader.View(std::size_t(count) * sizeof(PackedRenderVertexV12));
    ParallelRanges(count, [&](std::size_t first, std::size_t last) {
      for (std::size_t index = first; index < last; ++index) {
        PackedRenderVertexV12 source;
        std::memcpy(&source, bytes.data() + index * sizeof(source),
                    sizeof(source));
        RenderVertex& vertex = destination[first_destination + index];
        vertex.position = {source.base.position[0], source.base.position[1],
                           source.base.position[2]};
        vertex.normal = {source.base.normal[0], source.base.normal[1],
                         source.base.normal[2]};
        vertex.uv = {source.base.uv[0], source.base.uv[1]};
        vertex.material = source.base.material;
        vertex.lightmap_uv = {source.base.lightmap_uv[0],
                              source.base.lightmap_uv[1]};
        vertex.decal_uv = {source.decal_uv[0], source.decal_uv[1]};
        vertex.tangent_binormal = {DecodeSnorm8(source.packed_tangent, 0u),
                                   DecodeSnorm8(source.packed_tangent, 8u),
                                   DecodeSnorm8(source.packed_tangent, 16u)};
        vertex.tangent_handedness = DecodeSnorm8(source.packed_tangent, 24u);
      }
    });
    return;
  }
  ...
  ParallelRanges(count, [&](std::size_t first, std::size_t last) {
    for (std::size_t index = first; index < last; ++index) {
      ...
      std::memcpy(&source, bytes.data() + index * sizeof(source),
                  sizeof(source));
      RenderVertex& vertex = destination[first_destination + index];
      vertex.position = {source.position[0], source.position[1],
                         source.position[2]};
      ...
      vertex.lightmap_uv = {source.lightmap_uv[0], source.lightmap_uv[1]};
      vertex.decal_uv = vertex.uv;
    }
  });
}

void ReadIndices(Reader& reader, std::uint32_t count,
                 std::vector<std::uint32_t>& destination) {
  const std::size_t first_destination = destination.size();
  destination.resize(first_destination + count);
  reader.Bytes(destination.data() + first_destination,
               std::size_t(count) * sizeof(std::uint32_t));
}

void ReadCollisionTriangles(Reader& reader, std::uint32_t count,
  ...
  if (has_native_edge_codes) {
    const std::span<const std::uint8_t> bytes =
        reader.View(std::size_t(count) * sizeof(PackedCollisionTriangleV11));
    ParallelRanges(count, [&](std::size_t first, std::size_t last) {
      for (std::size_t index = first; index < last; ++index) {
        PackedCollisionTriangleV11 source;
        std::memcpy(&source, bytes.data() + index * sizeof(source),
                    sizeof(source));
        CollisionTriangle& triangle = destination[first_destination + index];
        triangle.a = {source.base.vertices[0], source.base.vertices[1],
                      source.base.vertices[2]};
        triangle.b = {source.base.vertices[3], source.base.vertices[4],
                      source.base.vertices[5]};
        triangle.c = {source.base.vertices[6], source.base.vertices[7],
                      source.base.vertices[8]};
        triangle.surface = source.base.surface;
        triangle.material = source.base.material;
        std::copy(std::begin(source.edge_codes), std::end(source.edge_codes),
                  triangle.native_edge_codes.begin());
        triangle.has_native_edge_codes = source.has_edge_codes != 0;
      }
    });
    return;
  }
  ...
  const std::span<const std::uint8_t> bytes =
      reader.View(std::size_t(count) * sizeof(PackedCollisionTriangleV9));
  ParallelRanges(count, [&](std::size_t first, std::size_t last) {
    for (std::size_t index = first; index < last; ++index) {
      ...
      std::memcpy(&source, bytes.data() + index * sizeof(source),
                  sizeof(source));
      CollisionTriangle& triangle = destination[first_destination + index];
      triangle.a = {source.vertices[0], source.vertices[1], source.vertices[2]};
      triangle.b = {source.vertices[3], source.vertices[4], source.vertices[5]};
      triangle.c = {source.vertices[6], source.vertices[7], source.vertices[8]};
...
const SurfaceMaterial* FindMaterial(const MapDefinition& map, MaterialId id) {
  const auto found = std::find_if(
      map.materials.begin(), map.materials.end(),
      [id](const SurfaceMaterial& material) { return material.id == id; });
  return found == map.materials.end() ? nullptr : &*found;
}

void ReadMapObjects(std::vector<std::uint8_t> payload, std::uint32_t schema,
                    MapDefinition& map) {
  if (schema < 1 || schema > 3) {
    throw std::runtime_error("SKATE MOBJ extension uses an unsupported schema");
  }
  ...
  std::vector<bool> claimed_collision(map.collision_triangles.size(), false);
  ...
  for (std::uint32_t object_index = 0; object_index < object_count;
    ...
    object.source_first_collision_triangle = reader.Scalar<std::uint32_t>();
    object.source_collision_triangle_count = reader.Scalar<std::uint32_t>();
    if (schema >= 2) {
      const std::uint32_t grind_count = reader.Scalar<std::uint32_t>();
      ...
      for (std::uint32_t grind = 0; grind < grind_count; ++grind) {
        const std::uint32_t rail_index = reader.Scalar<std::uint32_t>();
    ...
    if (schema >= 3) {
      const std::uint32_t physics_type = reader.Scalar<std::uint32_t>();
      const std::uint32_t collision_shape = reader.Scalar<std::uint32_t>();
      if (physics_type >
              static_cast<std::uint32_t>(ObjectPhysicsType::Dynamic) ||
          collision_shape >
              static_cast<std::uint32_t>(ObjectCollisionShape::ConvexHull)) {
        throw std::runtime_error("SKATE map object physics enum is invalid");
      }
      object.physics.type = static_cast<ObjectPhysicsType>(physics_type);
      object.physics.shape = static_cast<ObjectCollisionShape>(collision_shape);
      ...
      const std::uint32_t enable_sleep = reader.Scalar<std::uint32_t>();
      const std::uint32_t initially_awake = reader.Scalar<std::uint32_t>();
      if (enable_sleep > 1 || initially_awake > 1) {
        throw std::runtime_error("SKATE map object physics boolean is invalid");
      }
    ...
    if (object.id == 0 || object.name.empty() || !Finite(object.origin) ||
        object.source_index_count == 0 || object.source_index_count % 3u != 0 ||
        index_end > map.render_mesh.indices.size() ||
        collision_end > map.collision_triangles.size() ||
        !ids.insert(object.id).second || !names.insert(object.name).second) {
    ...
    for (std::uint64_t source_offset = object.source_first_index;
      ...
      if (claimed_indices[source_offset]) {
        throw std::runtime_error("SKATE map object render ranges overlap");
      }
      claimed_indices[source_offset] = true;
      const std::uint32_t source_index = map.render_mesh.indices[source_offset];
      ...
      const auto [found, inserted] = remap.emplace(
          source_index,
          static_cast<std::uint32_t>(object.render_mesh.vertices.size()));
      if (inserted) {
    ...
    object.collision_triangles.reserve(object.source_collision_triangle_count);
    for (std::uint64_t source_offset = object.source_first_collision_triangle;
         source_offset < collision_end; ++source_offset) {
      if (claimed_collision[source_offset]) {
        throw std::runtime_error("SKATE map object collision ranges overlap");
      }
      claimed_collision[source_offset] = true;
      CollisionTriangle triangle = map.collision_triangles[source_offset];
    ...
    object.local_bounds_min = object.render_mesh.vertices.front().position;
    object.local_bounds_max = object.local_bounds_min;
    const auto include = [&](Vec3 point) {
      object.local_bounds_min.x = std::min(object.local_bounds_min.x, point.x);
      object.local_bounds_min.y = std::min(object.local_bounds_min.y, point.y);
      object.local_bounds_min.z = std::min(object.local_bounds_min.z, point.z);
      object.local_bounds_max.x = std::max(object.local_bounds_max.x, point.x);
      object.local_bounds_max.y = std::max(object.local_bounds_max.y, point.y);
      object.local_bounds_max.z = std::max(object.local_bounds_max.z, point.z);
    };
...
void ReadBreakGroups(std::vector<std::uint8_t> payload, std::uint32_t schema,
                     MapDefinition& map) {
  if (schema != 1) {
    throw std::runtime_error("SKATE BGRP extension uses an unsupported schema");
  }
  if (map.editable_objects.empty()) {
    throw std::runtime_error("SKATE BGRP extension requires map objects");
  }
  Reader reader(std::move(payload));
  ...
  for (std::uint32_t index = 0; index < count; ++index) {
    ...
    const auto found =
        std::find_if(map.editable_objects.begin(), map.editable_objects.end(),
    ...
    if (found == map.editable_objects.end() ||
      ...
      throw std::runtime_error("SKATE BGRP object reference is invalid");
    }
    ...
    if (found->physics.break_group == 0) {
      throw std::runtime_error("SKATE BGRP group identifier must be nonzero");
    }
  }
  reader.RequireEnd();
}
...
void Validate(MapDefinition& map) {
  ...
  if (!cycle.enabled || !std::isfinite(cycle.duration_seconds) ||
      cycle.duration_seconds < 0.0f || !std::isfinite(cycle.start_time_hours) ||
      cycle.start_time_hours < 0.0f || cycle.start_time_hours >= 24.0f ||
      !std::isfinite(cycle.end_time_hours) || cycle.end_time_hours < 0.0f ||
      ...
      !Finite(cycle.moon_color) || !std::isfinite(cycle.sun_intensity) ||
      cycle.sun_intensity < 0.0f || !std::isfinite(cycle.moon_intensity) ||
      cycle.moon_intensity < 0.0f || !std::isfinite(cycle.day_ambient) ||
      cycle.day_ambient < 0.0f || cycle.day_ambient > 1.0f ||
      !std::isfinite(cycle.night_ambient) || cycle.night_ambient < 0.0f ||
      cycle.night_ambient > 1.0f || !Finite(cycle.sky_tint) ||
  ...
  if (map.materials.empty() || map.render_mesh.vertices.empty() ||
      map.render_mesh.indices.empty() || map.collision_triangles.empty()) {
    throw std::runtime_error(
  ...
  for (const ImageTexture& texture : map.textures) {
    ...
    const bool empty_placeholder = texture.width == 0 && texture.height == 0 &&
                                   texture.rgba8.empty() &&
                                   texture.stored_rgba8.empty();
    const bool decoded = texture.rgba8.size() == expected;
  ...
  if (textured_sky.enabled) {
    if (textured_sky.mesh.vertices.empty() ||
        ...
        textured_sky.detail_texture == 0 || textured_sky.sun_texture == 0 ||
    ...
    for (std::uint32_t index : textured_sky.mesh.indices) {
      if (index >= textured_sky.mesh.vertices.size()) {
        throw std::runtime_error("SKATE textured-sky index is out of range");
      }
    }
  }
  ...
  for (const SurfaceMaterial& material : map.materials) {
    if (material.id == 0 || material.name.empty() ||
        ...
        !Finite(material.display_color) || material.friction < 0.0f ||
        material.restitution < 0.0f || material.roughness < 0.0f ||
        material.roughness > 1.0f || material.baked_indirect_strength < 0.0f ||
        material.alpha_cutoff < 0.0f || material.alpha_cutoff > 1.0f ||
        static_cast<std::uint32_t>(material.alpha_mode) >
            static_cast<std::uint32_t>(SurfaceMaterial::AlphaMode::Blend) ||
        ...
        !std::isfinite(material.blend_factor) || material.blend_factor < 0.0f ||
        material.blend_factor > 1.0f || material.blend_mask_channel > 4 ||
        ...
        static_cast<std::uint32_t>(material.cull_mode) >
            static_cast<std::uint32_t>(SurfaceMaterial::CullMode::BackFaces)) {
    ...
    if (material.retail.enabled) {
      ...
      constexpr std::uint32_t kKnownRetailFlags = (1u << 7u) - 1u;
      if (material.retail.shader_name.empty() ||
          family > static_cast<std::uint32_t>(RetailShaderFamily::Sky) ||
          (flags & ~kKnownRetailFlags) != 0) {
        throw std::runtime_error("SKATE retail material metadata is invalid");
      }
      for (const RetailTextureBinding& binding :
           material.retail.texture_bindings) {
        if (binding.semantic.empty() || binding.texture == 0 ||
            !texture_ids.contains(binding.texture) || binding.uv_set > 2 ||
            binding.address_u > 2 || binding.address_v > 2) {
          throw std::runtime_error("SKATE retail texture binding is invalid");
        }
      }
  ...
  ParallelRanges(map.render_mesh.vertices.size(), [&](std::size_t first,
                                                      std::size_t last) {
    for (std::size_t index = first;
         index < last && !invalid_render_vertex.load(std::memory_order_relaxed);
         ++index) {
      const RenderVertex& vertex = map.render_mesh.vertices[index];
      if (!Finite(vertex.position) || !Finite(vertex.normal) ||
          !Finite(vertex.uv) || !Finite(vertex.lightmap_uv) ||
          !Finite(vertex.decal_uv) || !Finite(vertex.tangent_binormal) ||
          !std::isfinite(vertex.tangent_handedness) ||
          !material_ids.contains(vertex.material)) {
        invalid_render_vertex.store(true, std::memory_order_relaxed);
      }
    }
  });
  ...
  ParallelRanges(
      map.render_mesh.indices.size(), [&](std::size_t first, std::size_t last) {
        for (std::size_t position = first;
          ...
          if (map.render_mesh.indices[position] >=
            ...
            invalid_render_index.store(true, std::memory_order_relaxed);
          }
        }
      });
  ...
  ParallelRanges(map.collision_triangles.size(), [&](std::size_t first,
                                                     std::size_t last) {
    for (std::size_t index = first;
         index < last && !invalid_collision.load(std::memory_order_relaxed);
         ++index) {
      CollisionTriangle& triangle = map.collision_triangles[index];
      const Vec3 cross =
          Cross(triangle.b - triangle.a, triangle.c - triangle.a);
      if (!Finite(triangle.a) || !Finite(triangle.b) || !Finite(triangle.c) ||
          LengthSquared(cross) <= kAreaEpsilon || triangle.surface == 0 ||
          !material_ids.contains(triangle.material)) {
        invalid_collision.store(true, std::memory_order_relaxed);
        continue;
      }
  ...
  for (MapObject& object : map.editable_objects) {
    ...
    const bool valid_physics =
        ...
        object.physics.density > 0.0f && object.physics.density <= 100000.0f &&
        std::isfinite(object.physics.friction) &&
        object.physics.friction >= 0.0f && object.physics.friction <= 2.0f &&
    ...
    if (object.id == 0 || object.name.empty() ||
        ...
        !object_names.insert(object.name).second || !Finite(object.origin) ||
        !Finite(object.local_bounds_min) || !Finite(object.local_bounds_max) ||
        ...
        object.render_mesh.indices.size() % 3u != 0 || !valid_physics) {
    ...
    for (const RenderVertex& vertex : object.render_mesh.vertices) {
      if (!Finite(vertex.position) || !Finite(vertex.normal) ||
        ...
        throw std::runtime_error("SKATE editable map object vertex is invalid");
      }
    }
  ...
  for (const GrindRail& rail : map.grind_rails) {
    ...
    const bool points_valid =
        rail.points.size() >= 2 &&
        std::all_of(rail.points.begin(), rail.points.end(),
                    [](Vec3 point) { return Finite(point); });
    const bool native_valid =
        native && rail.points.empty() && rail.retail_spline_id != 0 &&
        rail.retail_type_signature != 0 &&
        std::all_of(rail.native_segments.begin(), rail.native_segments.end(),
    ...
    if (rail.id == 0 || rail.name.empty() || (!native && !points_valid) ||
        (native && !native_valid)) {
  ...
  for (const NpcRoute& route : map.npc_routes) {
    if (route.id == 0 || route.name.empty() ||
        !npc_route_ids.insert(route.id).second || route.points.size() < 2 ||
        route.skater_count == 0 || route.skater_count > 32 ||
        !std::isfinite(route.speed) || route.speed <= 0.0f ||
        route.speed > 30.0f || !std::isfinite(route.spawn_spacing) ||
  ...
  for (HingedDoor& door : map.hinged_doors) {
    ...
    if (door.id == 0 || door.name.empty() || !door_ids.insert(door.id).second ||
        !Finite(door.hinge_position) || !Finite(door.hinge_axis) ||
        !Finite(door.closed_width_axis) || !Finite(door.closed_depth_axis) ||
        ...
        std::abs(axis_dot) > 1.0e-3f || std::abs(depth_dot) > 1.0e-3f ||
        ...
        !std::isfinite(door.angular_damping) || door.angular_damping < 0.0f ||
        ...
        !std::isfinite(door.static_friction) || door.static_friction < 0.0f ||
        !std::isfinite(door.restitution) || door.restitution < 0.0f ||
  ...
  for (MovingLightOrb& light : map.moving_light_orbs) {
    if (light.id == 0 || light.name.empty() ||
        !light_ids.insert(light.id).second || !Finite(light.orbit_center) ||
        !Finite(light.direction) || !Finite(light.color) ||
        ...
        !std::isfinite(light.intensity) || light.source_radius <= 0.0f ||
        light.influence_radius <= light.source_radius ||
        light.intensity <= 0.0f || !std::isfinite(light.spot_inner_cosine) ||
        !std::isfinite(light.spot_outer_cosine) ||
        light.spot_inner_cosine < -1.0f || light.spot_inner_cosine > 1.0f ||
...
MapDefinition LoadOwnedMapPackage(const std::filesystem::path& path) {
  ...
  if (!stream) {
    throw std::runtime_error("could not open SKATE package: " + path.string());
  }
  ...
  return LoadOwnedMapPackage(bytes);
}

MapDefinition LoadOwnedMapPackage(std::span<const std::uint8_t> package_bytes) {
  if (package_bytes.empty() || package_bytes.size() > kMaximumPackageBytes) {
    throw std::runtime_error("SKATE package size is invalid");
  }
  Reader reader(
      std::vector<std::uint8_t>(package_bytes.begin(), package_bytes.end()));
  ...
  const bool version_10_or_newer = version_10 || version_11 || version_12 ||
                                   version_13 || version_14 || version_15;
  const bool skate_magic = std::memcmp(magic.data(), "SKATE", 5) == 0 &&
  ...
  if (skate_magic && package_version > kLatestSupportedOwnedMapPackageVersion) {
    throw std::runtime_error("SKATE v" + std::to_string(package_version) +
  ...
  if ((!version_1 && !version_2 && !version_3 && !version_4 && !version_5 &&
    ...
    throw std::runtime_error(
        ...
        std::to_string(kLatestSupportedOwnedMapPackageVersion) + " package");
  }
  ...
  if (version_15) {
    ...
    if (decoded_size > kMaximumMaterialBytes) {
      throw std::runtime_error("SKATE material block exceeds the format limit");
    }
    material_block.emplace(
        ReadStoredBytes(reader, decoded_size, "material block"));
  ...
  for (std::uint32_t index = 0; index < material_count; ++index) {
    ...
    material.flags =
        static_cast<SurfaceFlags>(material_reader->Scalar<std::uint32_t>());
    ...
    material.indirect_lightmap = material_reader->Scalar<TextureId>();
    material.baked_indirect_strength = material_reader->Scalar<float>();
    if (package_version >= 2) {
      material.normal_texture = material_reader->Scalar<TextureId>();
      material.orm_texture = material_reader->Scalar<TextureId>();
      material.emissive_texture = material_reader->Scalar<TextureId>();
      material.alpha_mode = static_cast<SurfaceMaterial::AlphaMode>(
          material_reader->Scalar<std::uint32_t>());
      material.alpha_cutoff = material_reader->Scalar<float>();
      material.skate_audio_surface =
          static_cast<std::uint8_t>(material_reader->Scalar<std::uint32_t>());
    ...
    if (package_version >= 13) {
      ...
      material.presentation_depth_layer = InferPresentationDepthLayer(material);
    }
    if (package_version >= 12) {
      material.retail.enabled = material_reader->Scalar<std::uint32_t>() != 0;
      if (material.retail.enabled) {
        ...
        material.retail.shader_family = static_cast<RetailShaderFamily>(
            material_reader->Scalar<std::uint32_t>());
        material.retail.render_flags = static_cast<RetailRenderFlags>(
            material_reader->Scalar<std::uint32_t>());
        ...
        for (std::uint32_t binding_index = 0; binding_index < binding_count;
             ++binding_index) {
          RetailTextureBinding binding;
          binding.semantic = material_reader->String();
          binding.texture = material_reader->Scalar<TextureId>();
          binding.uv_set = material_reader->Scalar<std::uint32_t>();
          binding.address_u = material_reader->Scalar<std::uint32_t>();
          binding.address_v = material_reader->Scalar<std::uint32_t>();
          material.retail.texture_bindings.push_back(std::move(binding));
        }
        ...
        for (std::uint32_t parameter_index = 0;
          ...
          for (std::uint32_t value_index = 0; value_index < value_count;
               ++value_index) {
            parameter.values.push_back(material_reader->String());
          }
          material.retail.parameters.push_back(std::move(parameter));
        }
        material.retail.source_metadata_json = material_reader->String();
      }
    }
  ...
  for (std::uint32_t index = 0; index < texture_count; ++index) {
    ...
    if ((texture.width == 0) != (texture.height == 0) ||
        (!empty_placeholder && (texture.width > kMaximumTextureDimension ||
                                texture.height > kMaximumTextureDimension)) ||
    ...
    if (package_version >= 9) {
      StoredPayload payload = ReadStoredPayload(
          reader, static_cast<std::size_t>(expected_byte_count),
          "embedded texture");
      if (payload.method == kStorageTextureReference) {
        if (!version_15 || payload.bytes.size() != sizeof(std::uint32_t)) {
          throw std::runtime_error(
              "SKATE embedded texture reference is invalid");
        }
        std::uint32_t source_index = 0;
        std::memcpy(&source_index, payload.bytes.data(), sizeof(source_index));
        ...
        texture.stored_rgba8_method = source.stored_rgba8_method;
  ...
  if (package_version >= 9) {
    Reader vertex_reader(ReadStoredBytes(
        ...
        "visual vertex block"));
    ReadRenderVertices(vertex_reader, vertex_count, map.render_mesh.vertices,
                       package_version >= 12);
    vertex_reader.RequireEnd();

    Reader index_reader(ReadStoredBytes(
        reader, std::size_t(index_count) * sizeof(std::uint32_t),
        "visual index block"));
    ReadIndices(index_reader, index_count, map.render_mesh.indices);
    index_reader.RequireEnd();

    Reader collision_reader(
        ReadStoredBytes(reader,
                        std::size_t(collision_count) *
                            (sizeof(float) * 9u + sizeof(std::uint32_t) * 2u +
                             (package_version >= 11 ? 4u : 0u)),
                        "collision block"));
    ReadCollisionTriangles(collision_reader, collision_count,
                           map.collision_triangles, package_version >= 11);
    ...
    ReadCollisionTriangles(reader, collision_count, map.collision_triangles,
                           package_version >= 11);
  }
  ...
  for (std::uint32_t index = 0; index < rail_count; ++index) {
    ...
    if (representation == 0) {
      ...
      const std::uint32_t segment_count = reader.Scalar<std::uint32_t>();
      ...
      throw std::runtime_error("SKATE grind representation is unsupported");
    }
  ...
  for (std::uint32_t index = 0; index < door_count; ++index) {
    ...
    const std::uint32_t door_vertex_count = reader.Scalar<std::uint32_t>();
    const std::uint32_t door_index_count = reader.Scalar<std::uint32_t>();
    const std::uint32_t door_collision_count = reader.Scalar<std::uint32_t>();
    ...
    for (std::uint32_t vertex_index = 0; vertex_index < door_vertex_count;
         ++vertex_index) {
      RenderVertex vertex;
    ...
    for (std::uint32_t door_index = 0; door_index < door_index_count;
         ++door_index) {
      door.render_mesh.indices.push_back(reader.Scalar<std::uint32_t>());
    }
  ...
  for (std::uint32_t index = 0; index < local_light_count; ++index) {
    ...
    light.type = static_cast<LocalLightType>(reader.Scalar<std::uint32_t>());
  ...
  if (package_version >= 12) {
    ...
    const std::uint32_t extension_count = reader.Scalar<std::uint32_t>();
    RequireCount(extension_count, "extension");
    for (std::uint32_t extension_index = 0; extension_index < extension_count;
         ++extension_index) {
      ...
      const bool supported_large_embedded_collision =
          tag == std::array<char, 4>{'R', 'W', 'C', 'M'} && schema == 1 &&
      ...
      if (decoded_size > kMaximumMetadataBytes &&
        ...
        if (tag == std::array<char, 4>{'W', 'M', 'E', 'T'} && schema == 1) {
          SkipStoredBytes(reader, decoded_size, "extension");
          continue;
        }
        throw std::runtime_error("SKATE extension exceeds the metadata limit");
      }
      std::vector<std::uint8_t> payload =
          ReadStoredBytes(reader, decoded_size, "extension");
      if (tag == std::array<char, 4>{'W', 'M', 'E', 'T'} && schema == 1) {
        map.retail_world_metadata_json.assign(
            reinterpret_cast<const char*>(payload.data()), payload.size());
      } else if (tag == std::array<char, 4>{'S', 'K', 'Y', 'B'} &&
                 schema == 1) {
        ...
        const std::uint32_t sky_index_count =
            sky_reader.Scalar<std::uint32_t>();
        RequireCount(sky_vertex_count, "textured-sky vertex",
                     kMaximumGeometryCount);
        RequireCount(sky_index_count, "textured-sky index", kMaximumIndexCount);
        if (sky_vertex_count == 0 || sky_index_count == 0 ||
            sky_index_count % 3 != 0) {
          throw std::runtime_error("SKATE textured-sky geometry is invalid");
        }
        ...
        const auto read_sky_texture = [&map, &sky_reader](const char* label) {
          ImageTexture texture;
          texture.id = static_cast<TextureId>(map.textures.size() + 1u);
        ...
        sky.gradient_texture = read_sky_texture("sky-gradient");
        sky.detail_texture = read_sky_texture("sky-detail");
        sky.sun_texture = read_sky_texture("sky-sun");

        Reader vertex_reader(ReadStoredBytes(
            sky_reader, std::size_t(sky_vertex_count) * kTexturedSkyVertexBytes,
            "textured-sky vertex block"));
        sky.mesh.vertices.reserve(sky_vertex_count);
        for (std::uint32_t index = 0; index < sky_vertex_count; ++index) {
          RenderVertex vertex;
          vertex.position = vertex_reader.Vector3();
          vertex.uv = vertex_reader.Vector2();
          const float length_squared = Dot(vertex.position, vertex.position);
          if (length_squared > kAreaEpsilon) {
            vertex.normal =
                vertex.position * (-1.0f / std::sqrt(length_squared));
          }
        ...
        Reader index_reader(ReadStoredBytes(
            sky_reader, std::size_t(sky_index_count) * sizeof(std::uint32_t),
            "textured-sky index block"));
        ReadIndices(index_reader, sky_index_count, sky.mesh.indices);
        index_reader.RequireEnd();
        sky_reader.RequireEnd();
        map.textured_sky = std::move(sky);
        map.sky.enabled = true;
      } else if (tag == std::array<char, 4>{'W', 'C', 'F', 'G'}) {
        ...
        if (dynamic_lighting > 1) {
          throw std::runtime_error("SKATE dynamic-lighting default is invalid");
        }
        map.dynamic_lighting_enabled_by_default = dynamic_lighting != 0;
        ...
        if (schema < 1 || schema > 3 || (schema == 3 && package_version < 14)) {
          throw std::runtime_error(
        ...
        constexpr std::array<std::uint8_t, 8> kArchiveMagic{'R', 'W', 'C', 'M',
                                                            'S', 'E', 'T', '1'};
        if (payload.size() < 12 ||
            !std::equal(kArchiveMagic.begin(), kArchiveMagic.end(),
                        payload.begin())) {
          throw std::runtime_error(
      ...
      } else if (tag == std::array<char, 4>{'B', 'M', 'A', 'T'} &&
                 schema == 1) {
        Reader material_reader(std::move(payload));
        const std::uint32_t count = material_reader.Scalar<std::uint32_t>();
        ...
        for (std::uint32_t material_index = 0; material_index < count;
             ++material_index) {
          const MaterialId material_id = material_reader.Scalar<MaterialId>();
          const auto found =
              std::find_if(map.materials.begin(), map.materials.end(),
          ...
          found->secondary_albedo_texture = material_reader.Scalar<TextureId>();
          found->blend_mask_texture = material_reader.Scalar<TextureId>();
          found->blend_factor = material_reader.Scalar<float>();
          found->blend_mask_channel = static_cast<std::uint8_t>(
              material_reader.Scalar<std::uint32_t>());
          found->albedo_address_mode = static_cast<std::uint8_t>(
              material_reader.Scalar<std::uint32_t>());
          found->secondary_address_mode = static_cast<std::uint8_t>(
              material_reader.Scalar<std::uint32_t>());
          found->blend_mask_address_mode = static_cast<std::uint8_t>(
              material_reader.Scalar<std::uint32_t>());
          found->cull_mode = static_cast<SurfaceMaterial::CullMode>(
              material_reader.Scalar<std::uint32_t>());
        }
        material_reader.RequireEnd();
      }
    ...
    if (break_group_seen) {
      ReadBreakGroups(std::move(break_group_payload), break_group_schema, map);
    }
  }
...
bool DecodeOwnedMapTexture(const ImageTexture& texture, std::string* error) {
  ...
  try {
    texture.rgba8 =
        DecodeStoredPayload(StoredPayload{texture.stored_rgba8_method, expected,
