...
This source tree contains the preservation-first extraction and Blender
conversion tools developed for the University map project. Retail inputs,
decoded textures, intermediate caches, `.blend` scenes, and `.skate` packages
remain local and are intentionally excluded from Git and releases.
...
The official Xbox 360 base-game `worldDIST_University.big` archive was
extracted from a legally owned copy into `raw/university/`. Its stream
contains 647 files totaling 798,135,872 bytes:
...
The Python preparation step currently uses the external UTT-1.1.7 RX2 parser.
Set `SKATE3_UTT_ROOT` to that checkout, or pass `-UttRoot` to the build script.
The parser is not bundled with this repository.

To rebuild without making a preview render:

```powershell
$env:SKATE3_UTT_ROOT = "D:\Tools\UTT-1.1.7"
...
and static lightmaps are transported independently. The current SKATE v12
owned package has:

- 8,729 materials, including 8,546 distinct retail draw definitions, and
  2,046 embedded textures;
- 6,572 opaque, 2,118 alpha-mask, and 39 alpha-blended material variants;
- 40,663 named retail texture bindings and 67,475 retained parameter entries;
- 2,987 mesh parts using 135 conventional retail normal maps;
- 8,489 mesh parts using 1,270 exact retail lightmap pages;
- 2,139,713 indexed visual vertices and 1,645,617 render triangles;
...
The SKATE v12 package is
`intermediate/university/University.skate` and is 254,686,430 bytes. Unlike
the 195,460,271-byte v11 baseline, it retains all eleven texture roles, every
retail material parameter, shader family, material GUID/handle/group,
decal UVs, packed tangent frame, and a compressed full extraction manifest.
It losslessly decodes to the counts and bounds above. The offline engine
validator also
...
writing SKATE v12. `verify_university_grinds.py` independently reconstructs
...
The manifest and v12 material table retain all known retail texture roles:
diffuse, transparent, normal, normal2, specular, lightmap, detail,
macro-overlay, decal, environment, and noise. The conventional normal slot
continues to bind 135 tangent-space maps to 2,987 mesh parts, while all 141
normal IDs—including seven shader-specific default/water/palm resources—also
remain available through named retail bindings. The exact-renderer route now
consumes the known environment, tree, decal, reflective, incandescent, water,
and ocean families. Cube-map array shape, animated scene instances, and the
small unsupported shader-family remainder still require reconstruction before
claiming 1:1 visual parity.
...
supplies the authored tangent. The importer preserves those values as hidden
Blender point attributes; the addon reconstructs the binormal required by the
SKATE runtime record instead of regenerating a tangent from Blender UVs.
`verify_university_world_frames.py` checks all 1,579,005 affected triangle
corners against that source-derived frame after export.
...
KD trees, cluster boundaries, unit flags, edge codes, and packed surfaces are
adopted directly. At runtime the nearest 12 resources are registered around
the skater and the active set is refreshed after 24 metres of movement. This
