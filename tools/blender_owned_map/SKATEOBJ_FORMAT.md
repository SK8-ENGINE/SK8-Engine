...
`.skateobj` is the spawnable-object profile of the existing `SKATE12` binary
package. It deliberately reuses SKATE's material, texture, render vertex,
...
- use the `.skateobj` filename extension and the `SKATE12\0` package magic;
...
The map-wide environment fields physically present in SKATE12 are ignored by
the object-profile loader and should be emitted as deterministic neutral
defaults. They are tiny compared with mesh and texture payloads, while reusing
them avoids a second incompatible parser. Exporters should include only
materials and textures referenced by the prefab.
