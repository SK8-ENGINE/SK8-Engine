...
[[nodiscard]] constexpr bool PoseControlTypeValid(
    PoseControlType type) {
  return type >= PoseControlType::kDecodedBaseline &&
         type <= PoseControlType::kRequestBaseline;
}

[[nodiscard]] constexpr bool PoseControlShapeValid(
    const PoseControl& control) {
  if (!PoseControlTypeValid(control.type) ||
      control.reserved_0 != 0 || control.reserved_1 != 0 ||
      control.target_role < 1 || control.target_role > 100 ||
      control.target_session == 0 || control.group_mask == 0) {
    return false;
  }
...
[[nodiscard]] constexpr bool PoseControlEnvelopeShapeValid(
    const Envelope& envelope, const PoseControl& control) {
...
[[nodiscard]] constexpr std::size_t PoseGroupFragmentOffset(
    std::uint8_t fragment_index) {
  return std::size_t(fragment_index) * kMaximumPoseFragmentBytes;
}

[[nodiscard]] constexpr std::size_t PoseGroupFragmentByteCount(
    std::uint32_t total_bytes, std::uint8_t fragment_index) {
  const std::size_t offset =
      PoseGroupFragmentOffset(fragment_index);
  if (offset >= total_bytes) {
    return 0;
  }
  const std::size_t remaining =
      std::size_t(total_bytes) - offset;
  return remaining < kMaximumPoseFragmentBytes
             ? remaining
             : kMaximumPoseFragmentBytes;
}

[[nodiscard]] constexpr bool PoseGroupHeaderShapeValid(
    const PoseGroupHeader& header, MessageKind kind) {
  ...
  if (header.pose_id == 0 || header.total_bytes == 0 ||
      ...
      header.group_id >= 32 ||
      !PoseGroupEncodingValid(header.encoding) ||
...
[[nodiscard]] constexpr bool PoseGroupEnvelopeShapeValid(
    const Envelope& envelope, const PoseGroupHeader& header) {
...
[[nodiscard]] inline bool EncodePoseControl(
    const PoseControl& control,
  ...
  const bool encoded =
      ...
      writer.U8(control.reserved_0) &&
      writer.U16(control.target_role) &&
      writer.U16(control.target_stream_id) &&
      writer.U16(control.reserved_1) &&
      writer.U32(control.target_session) &&
      writer.U32(control.baseline_id) &&
      writer.U32(control.group_mask);
  return encoded && writer.offset() == kPoseControlPayloadBytes;
}

[[nodiscard]] inline bool DecodePoseControl(
  ...
  std::uint8_t type = 0;
  if (!reader.U8(type) ||
      !reader.U8(decoded.reserved_0) ||
      !reader.U16(decoded.target_role) ||
      !reader.U16(decoded.target_stream_id) ||
      !reader.U16(decoded.reserved_1) ||
      !reader.U32(decoded.target_session) ||
      !reader.U32(decoded.baseline_id) ||
      !reader.U32(decoded.group_mask) ||
...
[[nodiscard]] inline bool EncodePoseGroupHeader(
  ...
  detail::LittleEndianWriter writer(
      destination.first(kPoseGroupHeaderBytes));
  const bool encoded =
      writer.U32(header.pose_id) &&
      writer.U32(header.baseline_id) &&
      writer.U32(header.total_bytes) &&
      writer.U32(header.fragment_offset) &&
      writer.U16(header.element_count) &&
      writer.U16(header.fragment_bytes) &&
      writer.U8(header.fragment_index) &&
      writer.U8(header.fragment_count) &&
...
[[nodiscard]] inline bool DecodePoseGroupHeader(
    std::span<const std::uint8_t> payload,
    MessageKind kind,
  ...
  detail::LittleEndianReader reader(
      payload.first(kPoseGroupHeaderBytes));
  PoseGroupHeader decoded;
  std::uint8_t encoding = 0;
  if (!reader.U32(decoded.pose_id) ||
      !reader.U32(decoded.baseline_id) ||
      ...
      !reader.U8(decoded.fragment_count) ||
      !reader.U8(decoded.group_id) ||
      !reader.U8(encoding) ||
      reader.offset() != kPoseGroupHeaderBytes) {
    return false;
  }
  decoded.encoding = static_cast<PoseGroupEncoding>(encoding);
  if (!PoseGroupHeaderShapeValid(decoded, kind) ||
      payload.size() !=
          std::size_t(kPoseGroupHeaderBytes) +
              decoded.fragment_bytes) {
    return false;
  }
  output = decoded;
  return true;
