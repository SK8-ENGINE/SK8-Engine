...
struct CompatibilityIdentity {
  ...
  operator==(const CompatibilityIdentity &) const = default;
};

[[nodiscard]] constexpr std::uint64_t Fnv1a64(std::string_view value) {
...
inline constexpr std::uint64_t kBuildCompatibilityHash =
    Fnv1a64("skate3-multiplayer-live-v12-predictive-exact-delta-1");
...
[[nodiscard]] constexpr Capabilities
MakeCapabilities(const CompatibilityIdentity &identity) {
  return {
...
[[nodiscard]] constexpr bool
CapabilityEnvelopeShapeValid(const Envelope &envelope) {
  return EnvelopeShapeValid(envelope) &&
         envelope.kind == MessageKind::kCapabilities &&
         envelope.flags == kFlagReliable && envelope.stream_id == 0 &&
