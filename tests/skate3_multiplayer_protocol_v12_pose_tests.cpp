...
void TestConstantsAndMessageKind() {
  Expect(kPoseControlPayloadBytes == 20,
         "pose control payload size changed");
  Expect(kPoseGroupHeaderBytes == 24,
         "pose group header size changed");
  Expect(kMaximumPoseFragmentBytes == 1136,
         "pose group fragment budget changed");
  Expect(kMaximumPoseGroupBytes == 65536,
         "pose group total budget changed");
...
void TestPoseControlGoldenBytesAndRoundTrip() {
  ...
  constexpr std::array<std::uint8_t, kPoseControlPayloadBytes>
      expected = {
          0x01, 0x00, 0x05, 0x00, 0x33, 0x22, 0x00,
          0x00, 0x04, 0x03, 0x02, 0x01, 0x14, 0x13,
  ...
  PoseControl decoded;
  Expect(DecodePoseControl(encoded, decoded),
         "pose control did not decode");
  Expect(decoded.type == control.type &&
             decoded.target_role == control.target_role &&
             decoded.target_stream_id ==
                 control.target_stream_id &&
  ...
  Expect(PoseControlEnvelopeShapeValid(
...
void TestPoseGroupGoldenBytesAndRoundTrip() {
  ...
  Expect(EncodePoseGroupHeader(
             header, MessageKind::kPoseBaseline, payload),
         "valid pose group header did not encode");
  constexpr std::array<std::uint8_t, kPoseGroupHeaderBytes>
      expected = {
          0x04, 0x03, 0x02, 0x01, 0x00, 0x00, 0x00, 0x00,
          0x73, 0x04, 0x00, 0x00, 0x70, 0x04, 0x00, 0x00,
          0x12, 0x01, 0x03, 0x00, 0x01, 0x02, 0x05, 0x01,
      };
  Expect(std::equal(
             expected.begin(), expected.end(), payload.begin()),
  ...
  Expect(DecodePoseGroupHeader(
             payload, MessageKind::kPoseBaseline, decoded),
...
void TestPoseGroupFragmentBoundaries() {
  ...
  header.fragment_count = static_cast<std::uint8_t>(
  ...
  Expect(PoseGroupHeaderShapeValid(
             header, MessageKind::kPoseDelta),
         "maximum bounded pose group fragment was rejected");
  Expect(header.fragment_count == 58 &&
             header.fragment_bytes == 784,
...
void TestPoseGroupMalformedDecode() {
  ...
  output.pose_id = 999;
  Expect(!DecodePoseGroupHeader(
             std::span<const std::uint8_t>(payload).first(
                 payload.size() - 1),
             MessageKind::kPoseBaseline, output),
         "truncated pose fragment was accepted");
  Expect(output.pose_id == 999,
         "failed pose group decode modified output");
  payload.push_back(0);
  Expect(!DecodePoseGroupHeader(
  ...
  Expect(!DecodePoseGroupHeader(
             payload, MessageKind::kRootSnapshot, output),
